...
<main>
    <section id="detail-event" class="detail-event">
        <div class="container">
            ...
            <p>Start Date <?= strftime("%A, %e %B %Y - %H:%M", strtotime($event['event_start_date'])); ?></p>
            <p>End Date <?= strftime("%A, %e %B %Y - %H:%M", strtotime($event['event_end_date'])); ?></p>
            <p>Form Link <a href="<?= $event['event_form_link']; ?>" target="_blank" rel="noopener noreferrer"><?= $event['event_form_link']; ?></a></p>
