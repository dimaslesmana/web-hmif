...
$('input:hidden[name="event_photo[]"]').each(function () {
  eventPhotos.push({
    source: `/assets/images/event-news/${this.value}`,
    options: {
      type: 'local'
    }
  })
...
filepondImageUpload.setOptions({
  ...
  server: {
    process: {
      url: '/dashboard/multiple-image-upload/upload',
      method: 'POST',
      onload: (response) => {
    ...
    revert: (uniqueFileId, load, error) => {
      $.ajax({
        url: `/dashboard/multiple-image-upload/delete/${uniqueFileId}`,
        type: 'DELETE',
        dataType: 'json',
    ...
    remove: (source, load, error) => {
      ...
      $.ajax({
        url: `/dashboard/multiple-image-upload/delete/${filename}`,
        type: 'DELETE',
        dataType: 'json',
        success: function (data, status, xhr) {
